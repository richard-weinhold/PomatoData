...
def get_availabilities_atlite(weather_year, cache_file_path, cache_file_name,
                              opsd_wind_power_plants_filepath,
                              countries):

    # weather_year = '2012'
    # cache_file_path = Path("C:/Users/riw/Documents/repositories/pomato_data/data_temp")
    # cache_file_name = "cwe"
    
    #read in onshore power plant data from OPSD
    onshore_df = read_in_opsd_data(opsd_wind_power_plants_filepath)
...
if __name__ == "__main__":
    ...
    opdf_filepath = Path(r"C:\Users\riw\Documents\repositories\pomato_2030\res_capacity\data\renewable_power_plants_DE.csv")
    countries = ["DE", "BE", "FR", "LU", "NL"]
    wind, pv = get_availabilities_atlite(2012, wdir.joinpath("data_temp"), "cwe", opdf_filepath, countries)
    # Save Resulting Tables. 
    wind.to_csv(wdir.joinpath('data_out/wind_availabilites.csv'))
    pv.to_csv(wdir.joinpath('data_out/pv_availabilites.csv'))
